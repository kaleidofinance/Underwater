...
async function readMarket(chain: Chain, launchpad: Address): Promise<MarketState> {
  ...
  // tick — `batch: true` folds same-tick reads into one POST.
  ...
  const [tokenCount, first] = await Promise.all([
    ...
    client.readContract({ ...common, functionName: "tokensSlice", args: [0n, BigInt(MARKET_LIMIT)] }) as Promise<readonly Address[]>,
  ]);

  const base = { chainId: chain.id, launchpad, tokenCount };
  if (tokenCount === 0n) return { ...base, listings: [], quotes: {} };

  // `tokensSlice` is newest-last, so the newest `MARKET_LIMIT` is the tail. When
  // there are more launches than the window they are not in this first read; the
  // count tells us before we waste a round on the wrong end.
  const past = tokenCount > BigInt(MARKET_LIMIT);
  ...
  const page = past
    ...
    : first;
  const tokens = page.slice().reverse();
  if (tokens.length === 0) return { ...base, listings: [], quotes: {} };
